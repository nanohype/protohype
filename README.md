...
| [watchtower](watchtower/)   | Regulatory change radar — crawls regulator feeds (SEC EDGAR, CFPB, OFAC, EDPB), classifies rule changes against per-client products × jurisdictions × frameworks via Bedrock Claude (fail-secure → human-review), drafts impact memos, publishes to Notion/Confluence behind an approval gate                          | worker-service, data-pipeline, module-vector-store, module-notifications-ts, module-knowledge-base-ts, eval-harness, infra-aws, @nanohype/cdk-constructs |
