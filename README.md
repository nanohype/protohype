...
| [almanac](almanac/)                 | Internal Slack knowledge bot over Notion, Confluence, Google Drive — per-user ACL enforced against each user's own OAuth tokens, hybrid k-NN+BM25 retrieval on OpenSearch Serverless, Bedrock Claude inference, KMS-encrypted token store, SQS+DLQ audit pipeline                                                      | slack-bot, rag-pipeline, module-vector-store, infra-aws                                           |
