...
async def run_summarize() -> dict:
    ...
    old_memories = await db.get_old_nonsummary_memories(older_than_iso=cutoff_iso, limit=settings.summarize_batch_size)

    if not old_memories:
        return {"summary_id": None, "memories_compressed": 0, "message": "No memories old enough to summarize"}

    summary_content = _build_summary_content(old_memories)
    summary_id = str(ULID())

    await db.insert_memory(id=summary_id, role="system", content=summary_content, tags=["summary", "auto-generated"], is_summary=True)

    from . import embeddings as emb
    vector = emb.embed(summary_content)
    await db.insert_embedding(id=str(ULID()), memory_id=summary_id, vector=vector, model=settings.embedding_model)

    old_ids = [m["id"] for m in old_memories]
    deleted = await db.delete_memories_by_ids(old_ids)

    msg = f"Summarized {deleted} memories older than {settings.summarize_min_age_hours}h into summary memory {summary_id}"
    logger.info(msg)

    return {"summary_id": summary_id, "memories_compressed": deleted, "message": msg}
