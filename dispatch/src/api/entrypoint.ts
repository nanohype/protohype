...
import { createEcsPipelineTrigger, type PipelineTriggerPort } from './pipeline-trigger.js';
...
const RuntimeEnvSchema = z.object({
  ...
  // Pipeline-trigger config (admin UI). All four must be present for the
  // /admin/pipeline-run routes to be wired; if any is missing the API
  // boots without the trigger and the routes 503.
  PIPELINE_CLUSTER_NAME: z.string().min(1).optional(),
  PIPELINE_TASK_DEF_FAMILY: z.string().min(1).optional(),
  PIPELINE_SECURITY_GROUP_ID: z.string().min(1).optional(),
  PIPELINE_SUBNET_IDS: z.string().min(1).optional(),
});
...
function maybeCreatePipelineTrigger(env: RuntimeEnv): PipelineTriggerPort | undefined {
  const { PIPELINE_CLUSTER_NAME, PIPELINE_TASK_DEF_FAMILY, PIPELINE_SECURITY_GROUP_ID, PIPELINE_SUBNET_IDS } = env;
  if (!PIPELINE_CLUSTER_NAME || !PIPELINE_TASK_DEF_FAMILY || !PIPELINE_SECURITY_GROUP_ID || !PIPELINE_SUBNET_IDS) {
    return undefined;
  }
  return createEcsPipelineTrigger({
    cluster: PIPELINE_CLUSTER_NAME,
    taskDefinitionFamily: PIPELINE_TASK_DEF_FAMILY,
    securityGroupId: PIPELINE_SECURITY_GROUP_ID,
    subnetIds: PIPELINE_SUBNET_IDS.split(',').map((s) => s.trim()).filter(Boolean),
  });
}
...
async function main(): Promise<void> {
  ...
  const app = await buildServer({
    ...
    pipelineTrigger: maybeCreatePipelineTrigger(env),
  });
