...
**Current state:** ✅ Lambda authorizer validates `x-api-key` header on all routes. API key auto-generated by CDK and stored in Secrets Manager (`mcp-switchboard/api-key`). Authorizer result cached for 5 minutes.

| Risk | Severity | Status |
|------|----------|--------|
| Unauthenticated tool calls | ✅ Resolved | Lambda authorizer on all routes |
| Stripe tools allow reading customer/payment data | ✅ Resolved | Blocked by API key |
...
- [x] Add API Gateway auth (Lambda authorizer with x-api-key header)
