...
export function createStripeServer(creds: { secretKey: string }): McpServer {
  ...
  server.registerTool(
    ...
    {
      description: 'List Stripe customers. Optionally search by email.',
      inputSchema: {
        limit: z.number().int().min(1).max(100).default(20).describe('Max customers to return'),
        email: z.email().optional().describe('Filter by email address'),
  ...
  server.registerTool(
    ...
    {
      description: 'Get details of a specific Stripe subscription.',
      inputSchema: {
  ...
  server.registerTool(
    'stripe_get_invoice',
    {
      description: 'Get details of a specific Stripe invoice.',
      inputSchema: {
