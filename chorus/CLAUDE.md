...
- OpenTelemetry is the shared observability surface. Every container is launched with `node --import ./dist/src/lib/telemetry-register.js` so auto-instrumentations (http, pg, express) patch before user code. Hot paths (pipeline stages, external HTTP, SDK calls) carry manual spans with `chorus.correlation_id`, `chorus.stage`, and `chorus.breaker.state` attributes. Metrics + traces go to Grafana Cloud via the ADOT sidecar on `localhost:4318`. Logs dual-ship: `observability.ts` writes JSON to stdout (picked up by the ECS awsLogs driver → CloudWatch) *and* emits via the OTel logs API (picked up by the ADOT sidecar → Grafana Loki), so trace↔log pivots work in Grafana while CloudWatch remains the audit trail.
